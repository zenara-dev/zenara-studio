      ...
      <div className="px-6 md:px-12 lg:px-16">
        <div className="grid grid-cols-12 gap-6 mb-12 md:mb-16">
          ...
          <h2 className="col-span-12 md:col-span-9 font-display text-3xl md:text-5xl lg:text-6xl tracking-tighter">
            What we do, <span className="text-[var(--zd-muted)]">end to end</span>.
          </h2>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 border-t border-l border-[var(--zd-border)]">
          {ITEMS.map((it) => {
            const Icon = it.icon;
            return (
              <article
                ...
                className="group relative border-r border-b border-[var(--zd-border)] p-8 md:p-10 bg-white hover:bg-[var(--zd-ink)] hover:text-white transition-colors duration-200"
              >
                <div className="flex items-start justify-between">
                  <span className="font-mono text-[11px] tracking-[0.24em] text-[var(--zd-muted)] group-hover:text-[var(--zd-accent)]">
                    0{it.id}
                  </span>
                ...
                <p className="mt-3 text-sm md:text-base leading-relaxed text-[var(--zd-muted)] group-hover:text-white/70">
                  {it.desc}
                </p>
              </article>
            );
