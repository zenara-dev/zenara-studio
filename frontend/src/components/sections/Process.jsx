      ...
      <div className="px-6 md:px-12 lg:px-16">
        <div className="grid grid-cols-12 gap-6 mb-12 md:mb-16">
          ...
          <h2 className="col-span-12 md:col-span-9 font-display text-3xl md:text-5xl lg:text-6xl tracking-tighter">
            Scoped weekly.{" "}
            <span className="text-[var(--zd-muted)]">Shipped calmly.</span>
          </h2>
        </div>

        <ol className="grid grid-cols-1 md:grid-cols-4 border-t border-l border-[var(--zd-border)]">
          {STEPS.map((s) => (
            <li
              key={s.n}
              data-testid={`process-step-${s.n}`}
              className="relative border-r border-b border-[var(--zd-border)] p-8 md:p-10 group hover:bg-[var(--zd-bg)] transition-colors"
            >
              ...
              <p className="mt-3 text-sm md:text-base leading-relaxed text-[var(--zd-muted)]">
                {s.desc}
              </p>
              <span className="absolute left-0 top-0 w-full h-[2px] bg-[var(--zd-accent)] scale-x-0 group-hover:scale-x-100 origin-left transition-transform duration-500" />
            </li>
          ))}
        </ol>
      </div>
